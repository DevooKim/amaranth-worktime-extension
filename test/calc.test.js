...
test("결재 대기 종일 휴가는 공휴일처럼 남은 근무일에서 빠진다", () => {
  const leaves = [
    { start: "20260904", end: "20260904", name: "연차", pending: true },
    { start: "20260908", end: "20260908", name: "오전반차", pending: true },
  ];
  assert.deepEqual([...fullDayLeaveDates(leaves)], ["20260904"]);
  const status = computeStatus({
    rows: [],
    holidays: ["20260924", "20260925", "20260926"],
    leaves,
  ...
  // 기본 20일에서 9/4 종일 연차만 빠진다. 반차는 근무일로 남긴다.
  assert.equal(status.remainingWorkDays, 19);
  assert.equal(status.monthWorkDays, 19);
});

test("달력에 결재 대기 휴가가 붙는다", () => {
  ...
  const cell = cal.weeks.flat().find((c) => c?.date === "20260921");
  assert.equal(cell.leaveName, "리프레시휴가");
  assert.equal(cell.pending, true);
  assert.equal(cell.isWorkday, false);
});
...
test("남은 근무일은 이번 달 전체 근무일과 함께 나온다", () => {
  ...
  const rows = ["20260901", "20260902", "20260903"].map((d) =>
    workday(d, "0900", "1800", 480),
  );
...
test("다른 날 더 일한 시간이 오늘 목표에서 빠진다", () => {
  ...
  const rows = [
    workday("20260901", "0900", "2000", 600),
    workday("20260902", "0900", "2000", 600),
  ];
  const status = computeStatus({
    rows,
    holidays: [],
  ...
  assert.ok(
    status.todayTarget < flat.todayTarget,
    "초과분만큼 오늘 목표가 줄어야 한다",
  );
  assert.equal(
...
test("달력 칸에 근무·휴가·미등록이 담긴다", () => {
  ...
  assert.equal(
    cells["20260922"].leaveName,
    "연차",
    "기록이 없어도 캘린더 휴가가 붙는다",
  );
  assert.equal(
    cells["20260922"].isWorkday,
    false,
    "종일 휴가는 근무일이 아니다",
  );
...
test("근무일 수의 근거를 설명할 수 있다", () => {
  ...
  const bd = workDayBreakdown(
    "20260901",
    new Set(["20260924", "20260925", "20260926"]),
  );
  assert.equal(bd.weekdays, 22, "9월 월~금은 22일");
  assert.equal(
    bd.holidayWeekdays,
    2,
    "추석 3일 중 평일은 24·25 이틀 (26일은 토요일)",
  );
...
test("예전 스키마로 저장된 결과를 그려도 NaN 이 새어 나오지 않는다", () => {
  ...
  assert.equal(
    s.todayRemainingByDaily,
    180,
    "480 − 300. 0 이 되어 다 채웠다고 하면 안 된다",
  );
...
test("팀 근태 달력은 날짜마다 쉬는 사람을 모은다", () => {
  ...
  assert.equal(
    cells["20260904"].people[0].person,
    "나",
    "내 일정이 맨 앞에 온다",
  );
