 ...
 */
async function request(pathname, { body, credentials, sendCookies = false, headers = {} }) {
...
function postForm(pathname, params, credentials) {
  return request(pathname, {
 ...
 */
export async function fetchLeaves(credentials, identity, { from, to }) {
  // oAuthToken 이 'groupSeq|empSeq|랜덤' 형식이라 식별 정보가 없어도 여기서 뽑을 수 있다.
  // 덕분에 예전 버전에서 저장한 식별 정보만 있어도 휴가 조회가 된다.
...
export async function fetchCompanyInfo(credentials) {
  const data = await postForm('/gw/gw050A02', { a10Domain: ORIGIN }, credentials);
