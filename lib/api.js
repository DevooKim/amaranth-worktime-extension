...
import { workedMinutes } from "./calc.js";
import {
  mergeLeaves,
...
export async function readCredentials() {
  const [token, signKey] = await Promise.all([
...
async function sign({ signKey, token, transactionId, timestamp, pathname }) {
  ...
  const buf = await crypto.subtle.sign(
    "HMAC",
    key,
    enc.encode(token + transactionId + timestamp + pathname),
  );
...
async function request(
  pathname,
  { body, credentials, sendCookies = false, headers = {}, okResultCodes = [0] },
) {
  ...
  if (res.status === 401 || res.status === 403)
    throw new AuthError("그룹웨어 세션이 만료되었습니다.");
  ...
  try {
    ...
    throw new Error(`${pathname} 응답이 JSON 이 아닙니다`);
  }
  const ok = okResultCodes.some(
    (code) => String(code) === String(json.resultCode),
  );
  if (!ok) {
...
/** 근태신청(HPD0110) 메뉴 코드가 필요한 호출. */
export function postAttend(pathname, body, credentials) {
  return post(pathname, body, credentials, { "menu-code": "HPD0110" });
}
...
function seqsFromToken(token) {
  ...
  if (!groupSeq || !empSeq)
    throw new AuthError("oAuthToken 형식을 해석할 수 없습니다.");
...
export async function fetchLeaves(credentials, identity, range) {
  const { empSeq, list } = await fetchAttendanceCalendar(
    credentials,
    identity,
    range,
  );
...
/** 결재 진행 중인 근태신청. HPD0110 캘린더와 같은 경로·서명. */
export async function fetchPendingLeaves(credentials, { coCd, from, to }) {
  try {
    const data = await post(
      "/human/attendapplication/at00001",
      {
        approStateList: ["0"],
        linkAtCdList: PENDING_LINK_AT_CDS,
        startDate: from,
        endDate: to,
    ...
    const parsed = parsePendingCalendarRows(data);
  ...
  const fallback = await post(
    ...
    { coCd, startDt: from, endDt: to, flag: "1" },
    credentials,
    { "menu-code": "HPD0120" },
  );
  return parsePendingApplications(fallback);
}

/** 캘린더 휴가에 결재 대기 건을 합친다. */
export async function fetchLeavesWithPending(credentials, identity, range) {
  const [cal, pending] = await Promise.all([
    fetchLeaves(credentials, identity, range).catch(() => []),
    fetchPendingLeaves(credentials, {
      coCd: identity.coCd,
      from: range.from,
      to: range.to,
    }).catch(() => []),
  ]);
  return mergeLeaves(cal, pending);
}
...
export async function fetchTeamLeaves(credentials, identity, range) {
  ...
  return list
    ...
    .sort(
      (a, b) =>
        a.start.localeCompare(b.start) || a.person.localeCompare(b.person),
    );
}
...
export async function markAlertsRead(credentials, companyInfo, alertIds) {
  ...
  await postWithCookies(
    "/event/event02A03",
    {
      header: { groupSeq, empSeq, pid: "", tid: "" },
      body: { companyInfo, alertIds: ids },
    },
    credentials,
  );
}

/** 받은 알림을 모두 읽음 처리한 뒤 안 읽음 건수를 갱신한다. */
export async function markAllAlertsRead(credentials) {
  const { groupSeq, empSeq } = seqsFromToken(credentials.token);
  const header = { groupSeq, empSeq };
  await postWithCookies(
    "/event/event02A04",
    { header, body: { mentionYn: "ALERT", eventType: "" } },
    credentials,
  );
  await postWithCookies(
    "/event/event02A12",
    { header, body: { eventType: "" } },
    credentials,
  );
}
...
export async function fetchRoster(credentials) {
  ...
  for (const r of rows || []) {
    ...
    if (r.deptName && !cur.depts.includes(r.deptName))
      cur.depts.push(r.deptName);
  ...
  return [...byEmp.values()]
    ...
    .sort(
      (a, b) =>
        a.dept.localeCompare(b.dept) || a.person.localeCompare(b.person),
    );
}

/**
 * 팀원 여러 명의 오늘 출근/퇴근 시각.
 * 오늘 출퇴근 API(getTodayComeLeaveInfo)를 각 사번으로 부른다.
 * 일자별 기록 API 와 달리 '진행 중인 오늘'도 실시간으로 준다.
 * members: [{ name, empCd }]
 */
export async function fetchTeamAttendance(
  ...
  const list = members || [];
  // 'yyyyMMddHHMM' 또는 'HHMM' → 'HHMM'. 값이 없으면 ''.
  const hhmm = (v) => {
  ...
  const results = await Promise.all(
    list.map(async (m) => {
      try {
        const res = await fetchTodayCommute(credentials, {
          empCd: m.empCd,
          coCd,
          workDt: date,
        });
        const d = res || {};
        const come = hhmm(d.comeTm);
        const leave = hhmm(d.leaveTm);
        const rawLeave = String(d.atNm || "").trim();
        const leaveName = rawLeave && rawLeave !== "-" ? rawLeave : null;
        // 근무시간은 퇴근까지 찍혔을 때만. 휴게는 기본 12–13시를 겹치는 만큼 뺀다.
        let workMin = 0;
        const c = toMin(come),
          l = toMin(leave);
        if (c != null && l != null && l > c) {
          workMin = workedMinutes(c, l);
        }
        return {
          name: m.name,
          empCd: m.empCd,
