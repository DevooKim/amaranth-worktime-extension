...
function heroContent(status, view) {
    ...
    return {
      ...
      pill:
        status.remainingWorkDays > 0
          ? `남은 ${status.remainingWorkDays}일`
          : "",
      sub: `소정 ${formatDuration(status.monthStandard)}${
        status.shortage > 0
          ? ` · ${formatDuration(status.shortage)} 부족`
          : " · 다 채웠어요"
      }`,
      bar: {
        fill: monthPct,
  ...
  return {
    ...
    value,
    pill:
      status.state !== "done" && leaveAt != null
        ? `${formatClock(leaveAt)} 퇴근`
        : "",
    sub,
    bar: dayBar,
  };
...
function renderToday(rawStatus, fetchedAt, staleNote) {
  ...
  $("today-remaining").textContent =
    status.todayRemainingByDaily > 0
  ...
  if (eff.balance == null) {
    ...
    balanceEl.textContent =
      rounded === 0
        ? "0분"
        : `${rounded > 0 ? "+" : "−"}${formatDuration(Math.abs(rounded))}`;
  }
...
function renderLeaves(leaves = []) {
  ...
  for (const leave of leaves) {
    ...
    day.textContent =
      from === to ? labelDate(from) : `${labelDate(from)} ~ ${labelDate(to)}`;
...
function renderAnnualLeave(annual) {
  ...
  $("lb-used").textContent =
    `총 ${formatDays(annual.total)}일 중 ${formatDays(annual.used)}일 사용`;
...
function renderDayDetail(cell) {
  ...
  title.textContent =
    labelDate(cell.date) + (cell.holidayName ? ` · ${cell.holidayName}` : "");
...
function failureText(res) {
  return (
    REASON_TEXT[res?.reason] || res?.message || "기록을 가져오지 못했어요."
  );
}
...
function fillDeptOptions(leaves, myDept) {
  ...
  if (myDept)
    quick.push({
      value: "mine",
      name: `우리 팀 · ${myDept}`,
      count: counts.get(myDept) || 0,
    });
  if (teamGroup.length) {
    quick.push({
      value: "group",
      name: `내 그룹 · ${teamGroup.length}명`,
      count: groupCount,
    });
  }
...
function setupTabs() {
  for (const tab of document.querySelectorAll(".tab")) {
    tab.addEventListener("click", () => {
      ...
      if (tab.dataset.tab === "records" && viewMonth == null)
        showMonth(thisMonth);
      if (tab.dataset.tab === "team" && teamMonth == null)
        showTeamMonth(thisMonth);
...
async function loadCrew() {
  ...
  if (!res || !res.ok) {
    ...
    $("crew-empty").textContent =
      failureText(res) || "출근 현황을 불러오지 못했어요.";
    return;
  }
...
function renderUpdate(info) {
  ...
  $("ver-now").textContent =
    `v${info?.current ?? chrome.runtime.getManifest().version}`;
  tag.hidden = false;
  ...
  if (info.behind) {
    ...
    note.textContent =
      "업데이트를 누르면 저장된 설치 폴더에 적용하고 자동으로 다시 로드합니다. 폴더가 없으면 먼저 지정합니다.";
    get.hidden = false;
    return;
  }
  ...
  tag.textContent = "최신";
  const when = info.checkedAt
    ? new Date(info.checkedAt).toTimeString().slice(0, 5)
    : null;
  note.textContent = when
    ? `최신 버전이에요. ${when} 에 확인했어요.`
    : "최신 버전이에요.";
}
...
function renderAlertPoll(lastPoll) {
  ...
  const detail = lastPoll.baseline
    ? "기준선 설정"
    : `새 알림 ${lastPoll.notified}건` +
      (lastPoll.failed ? ` (${lastPoll.failed}건 실패, 다음에 재시도)` : "");
  el.textContent = `3분마다 자동 확인 · 마지막 ${at} · ${detail}`;
}

function noteRead(ids) {
  lastAlerts = markLocallyRead(lastAlerts, ids);
...
function openAlert(alert) {
  ...
  if (alert.alertId && isUnread(alert)) noteRead([alert.alertId]);
}
...
function renderAlertList(alerts, moreYn) {
  ...
  for (const alert of alerts) {
    const li = document.createElement("li");
    if (alert.alertId) li.dataset.alertId = String(alert.alertId);
    ...
    meta.textContent = [alert.eventType, formatAlertTime(alert.createDate)]
      .filter(Boolean)
      .join(" · ");
    ...
    readBtn.addEventListener("click", async () => {
      ...
      noteRead([id]);
      const res = await ask({ type: "markAlertsRead", alertIds: [id] });
      if (res.ok) return;
      lastAlerts = snapshot;
      renderAlertList(snapshot, more);
      const row = $("alert-list").querySelector(
        `[data-alert-id="${CSS.escape(String(id))}"]`,
      );
      if (!row) return;
      const retry = row.querySelector(".alert-read");
      if (retry) {
        retry.disabled = false;
        retry.title = "다시 시도";
        retry.setAttribute("aria-label", "다시 시도");
      }
      const why = document.createElement("span");
      why.className = "alert-error";
      why.textContent = res.message || "읽음 처리를 못 했어요.";
      row.querySelector(".alert-error")?.remove();
      row.appendChild(why);
    });
...
async function loadAlerts({ force = false } = {}) {
  ...
  try {
    ...
    res = {
      ok: false,
      reason: "error",
      message: `화면 그리기 실패: ${err?.message || err}`,
...
document.addEventListener("click", (e) => {
  if (!$("scope-menu").hidden && !e.target.closest(".team-filter"))
    closeScopeMenu();
});
document.addEventListener("keydown", (e) => {
...
$("crew-add-code").addEventListener("click", () => {
  ...
  if (!found.length) {
    crewMsg("공유 코드나 사번을 찾지 못했어요.", false);
    return;
  }
...
savedFolder()
  .then((handle) => {
    $("ver-folder-check").hidden = !handle;
  })
  .catch(() => {
    $("ver-folder-check").hidden = true;
  });
...
chrome.storage.local
  .get(INCLUDE_TODAY_KEY)
  .then(({ [INCLUDE_TODAY_KEY]: saved }) => {
...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area !== "session" || !changes.lastAlerts) return;
  const next = changes.lastAlerts.newValue;
  if (!next || !Array.isArray(next.alerts)) return;
  alertsLoaded = true;
  renderAlertList(next.alerts.filter(Boolean), next.moreYn);
});
...
tickTimer = setInterval(() => {
  ...
  current.todayRemainingByDaily = Math.max(
    0,
    current.dailyMinutes - current.todayProgress,
  );
  current.shortage = current.monthStandard - current.accumulated;
  current.progressRatio =
    current.monthStandard > 0
      ? Math.min(1, current.accumulated / current.monthStandard)
      : 0;
  renderToday(current);
}, 60_000);
